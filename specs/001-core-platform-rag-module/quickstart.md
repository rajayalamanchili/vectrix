...
2. **US1 threshold empties results (new, FR-013/SC-007)** -- on the
   Retrieval step, raise the similarity-threshold slider toward `1.00`
   until the retrieved list becomes empty and the chart shows no
   highlighted chunk.
3. **US1 chunking-strategy changes boundaries (new, FR-014/SC-008)** --
   on the Chunking step, toggle from fixed-size to sentence-boundary at
   the same chunk-size value and confirm the chunk boundaries visibly
   differ.
4. **US1 document-switch reset (new, per `/speckit.clarify`)** -- get
   ...
   to Document and the previous query/results no longer appear anywhere
   downstream.
5. **US2 home page discovery** -- load `/`, confirm the RAG module card
   shows title/tagline/category/time and navigates correctly.
6. **US3 compare variants** -- open Compare Variants, select two
...
```bash
npm run check:extensibility   # SC-002 -- exits 0 if no cross-module conditionals
...
All six manual scenarios behave as described, and all four `npm run
check:*` commands exit `0`. That combination is what closes Milestone
1's Definition of Done per roadmap.md (as corrected 2026-08-03): the
SC-002/003/005 automated-check gap, the SC-006 re-verification gap, and
the FR-013/FR-014 build gap, together.
