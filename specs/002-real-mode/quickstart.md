...
3. **US2 real embedding chart** -- reach the Embedding step with Real
   ...
   ("PCA") is named (Acceptance Scenarios 1, 3).
4. **US2 embedding failure fallback** -- temporarily use an invalid key,
   ...
   Simulated Mode (Acceptance Scenario 2).
5. **US3 custom document** -- paste a custom document and question,
   ...
   Scenarios 1-2).
6. **US4 real generation + temperature** -- reach Generation with Real
...
7. **US5 HyDE for real** -- in Compare Variants with Real Mode active,
...
8. **US5 RAG-Fusion for real** -- select RAG-Fusion, confirm each query
...
9. **US5 non-executable variants labeled honestly** -- select GraphRAG,
   Self-RAG, or Agentic RAG in Real Mode, confirm the UI states plainly
   these remain explanatory-only rather than implying real execution
   (Scenario 4).
10. **US6 recall@K** -- define at least one (question, expected chunk)
    pair, run an evaluation, confirm a recall@K score (with method named
    in the UI) appears per configuration, side by side, and confirm
    changing the pipeline's Top-K changes the recall@K score shown
    (FR-011).
...
Milestone 1's close. All ten manual scenarios above behave as described
against a real OpenAI key. All seven `npm run check:*` commands exit
`0`. That combination is what closes spec.md 002's Definition of Done
per roadmap.md: every acceptance scenario passes, SC-001/SC-002
(002-spec) confirm Milestone 1 is unaffected, and SC-006 is verified --
not assumed -- via `check:key-isolation`.
